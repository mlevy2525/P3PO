...
def _load_files(demo_path):
    ...
    image_indices_path = os.path.join(
        demo_path, f"image_indices_cam_{CAM_ID}.pkl"
    )
...
if __name__ == "__main__":
    ...
    for demo_num, object_keypoint_trajectory in object_keypoints.items():
        ...
        if f"demonstration_{demo_num}" not in os.listdir(args.preprocessed_data_dir) or not os.path.exists(hand_trajectory_path + f"/image_indices_cam_{CAM_ID}.pkl"):
        ...
        for start_idx in range(args.subsample):
            ...
            for idx in range(start_idx, len(dataset[episode_idx]['actions']) + 1, args.subsample):
                concatenated = np.concatenate([
                    ...
                    dataset[episode_idx]['hand_keypoints'][idx].reshape(-1)
                ])
