...
class P3POWrapper(dm_env.Environment):
    def __init__(self, env, pixel_keys, depth_keys, training_keys, points_class, closed_loop_dataset_path, joint_space_control):
        self._env = env
        ...
        self.joint_space_control = joint_space_control
    ...
    def reset(self, **kwargs):
        ...
        if self.joint_space_control:
            obs['graph'] = torch.concatenate([obs['graph'][:points_dimensions], torch.tensor(obs['features'])])
        else:
    ...
    def step(self, action):
        ...
        if self.joint_space_control:
            obs['graph'] = torch.concatenate([obs['graph'][:points_dimensions], torch.tensor(obs['features'])])
            img = self.points_class.plot_image()[-1]
            ...
            if self.nearest_neighbor_state or self.save_nearest_neighbor_info:
                ...
                min_episode_idx = 0
                for episode_idx, episode in enumerate(self.closed_loop_dataset['observations']):
                    graph = torch.tensor(np.array(episode['graph']))
                    curr_graph = obs['graph'].unsqueeze(0)
                    min_idx = torch.abs(curr_graph - graph).sum(-1).argmin()
                    distance = torch.abs(curr_graph - graph).sum(-1)[min_idx]
                    if distance < min_distance:
